...
- **The National Trust source is gone, and the refusal it met is not.**
  ...
  **measured 30 Aug 2026 on the house server, robots.txt allows those
  ...
  site.
- **The Landmark Trust was measured and deliberately not added**, both
  halves of it. Its site refuses us: measured 20 Sep 2026 from a sandbox
  that had egress that day (Chenies answered 200 in the same minute, so
  this is the site and not the proxy), `www.landmarktrust.org.uk` returns
  **403 with `cf-mitigated: challenge`** and a Cloudflare "Just a moment…"
  interstitial on `/`, `/whats-on/`, `/sitemap.xml` and every `wp-json`
  route — `fetch.looks_like_a_challenge` true on all of them — while
  `robots.txt` answers 200, disallows none of those paths and advertises
  the sitemap it will not then serve. Yes in robots.txt and no at the
  door is the National Trust case again, so there is no events source and
  nothing renders past it.
  Wikidata can supply the buildings without the site, and the working
  query is recorded here because finding it is the part worth not
  repeating: they are linked by **ownership, not operator** —
  `?item (wdt:P137|wdt:P127) wd:Q2689124` — where `wdt:P137` alone finds
  4 and `wdt:P127` finds 25, so the National Trust's question would have
  returned four buildings and looked like a working query rather than a
  zero. The union is **29 with coordinates**, all `historic-house`, 27 UK
  mainland plus Fort Clonque (Alderney) and Nicolle Tower (Jersey).
  It was written, run against the live endpoint, and then removed: **29
  buildings is not worth a standing query**, and they are the wrong 29.
  A Landmark Trust building is holiday accommodation let by the week and
  its open days appear only on the site that challenges us, so every pin
  would be somebody's holiday let — a weaker claim than the NGS rule
  allows itself, where a garden with no future open day is not published
  at all. If the site ever stops challenging, the open days are what
  would make this worth revisiting, and the query above is the shortcut
  back in.
